...
import { createBooking, createEnrollmentWithAddress,
  createHotel, createPayment, createRoomWithHotelId,
  createTicket, createTicketTypeRemote, createUser,
  createTicketTypeWithHotel, 
  createBusyRoom,
  createTicketTypeNoHotel } from "../factories";
...
describe("GET/ booking", () => {
  ...
  it("should respond with status 200 and data if user has booking", async () => {
    const user = await createUser();
    const token = await generateValidToken(user);
...
describe("POST/ booking", () => {
  ...
  describe("When token is valid", () => {
    it("should respond with status 404 when roomId not exist", async () => {
      ...
      const response = await server.post("/booking").set("Authorization", `Bearer ${token}`).send({ roomId: 10000 });
    ...
    it("should respond with status 403 when room has no vacancy", async () => {
      ...
      const hotel = await createHotel();
      const room = await createBusyRoom(hotel.id);
    ...
    it("Should respond with status 200 and data booking", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
...
describe("PUT/ booking", () => {
  ...
  describe("When token is valid", () => {
    ...
    it("should respond with status 404 if room not exist", async () => {
      ...
      const response = await server.put("/booking/1").set("Authorization", `Bearer ${token}`).send({ roomId: 10000 });

      expect(response.status).toEqual(httpStatus.NOT_FOUND);
    });

    it("should respond with status 200 and data if update with sucess booking", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
